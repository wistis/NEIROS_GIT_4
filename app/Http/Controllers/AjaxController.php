...
class AjaxController extends Controller
{
    public function router($tip, Request $request)
    {
        ...
        switch ($tip) {
            ...
            case 'vkgroup':
                ...
                return VkApiController::vkgrouptokens($request);
            ...
            case 'load_tarif':
                ...
                return $this->load_tarif($request);
    ...
    public function set_terif($req){
$tariff=Tarifs::find($req->new_tarif);
$count_mont=1;
$total_summ=$tariff->month;
if($req->has('podpiska')){
$count_month=$req->{'radio-sale-card'};
if($count_month==3){
    $field='montsh3';
}if($count_month==6){
        $field='month6';
    ...
    $total_summ=($tariff->month- $tariff->month*$tariff->$field/100)*$count_month;
}
    /**/
        $my_company=auth()->user()->getMyCompany;

if($total_summ<=$my_company->ballans){
    $my_company->tariff_id=$tariff->id;
    $my_company->bay_tariff_id=$tariff->id;
    $my_company->is_active=1;
    $my_company->test_period=0;
    ...
    public function load_tarif($request){
$text='';
$my_terif=auth()->user()->getMyCompany->tariff_id;
$item=Tarifs::start_tarif();
$text.=view('billing.tariff_card',compact('item','my_terif'))->render();
$max_sort=Tarifs::where('month','>',0)
    ->where(function ($query)use($request) {

        $query  ->orwhere(function ($query) use ($request) {
            $query->where('projects', '<=', $request->sites)
                ->where('project_max', '>=', $request->sites);
        ...
        orwhere(function ($query) use ($request) {
            $query->where('traffic','<=',$request->traffic)
                ->where('traffic_max','>=',$request->traffic);
    ...
    ->where('for_all',1)->where('satatus',1)->max('sort');

$terifs=Tarifs::where('month','>',0)->where('sort',$max_sort)

    ->where('for_all',1)->where('satatus',1)->take(3)->get();
foreach ($terifs as $item){
    $text.=view('billing.tariff_card',compact('item','my_terif'))->render();
...
//print_r($sendArray);
    ...
    public function generatecallback(){
        ...
        whereHas('widget',function ($query){
            $query->wherein('tip',[1,3,19]);

        })->
        where('created_aster_call','!=',1) ->get();
        ...
        foreach ($projects as $project) {
            ...
            $data['input_calls'] = DB::connection('asterisk')->table('callback_calls')->where('callback_id', $project->call_back_random_id)->orderby('id', 'desc') ->where('in_new_table', '!=', 1) ->get();
            ...
            foreach ($data['input_calls'] as $item) {
                    ...
                    $newdata=AsteriskCall::where('my_company_id',$project->my_company_id)->where('unique_id',$item->callback_id)->first();
                    if(!$newdata) {

                        $newdata = new AsteriskCall();
                        $newdata->my_company_id =$project->my_company_id;
                        ...
                        $m=$prjc->get_audio_comment($item->callback_id);
                        if(!is_null($m['project_id'])){


                            $newdata->comment_op=$m['operator'];

                            $newdata->comment_cl=$m['client'];
                            $newdata->project_id=$m['project_id'];
                            $newdata->save();
                        }
        ...
        $kkal= DB::connection('asterisk')->table('callback_calls') ->orderby('id', 'desc') ->where('in_new_table', '!=', 1) ->get();
        foreach ($kkal as $item){
            $project=Project::where('call_back_random_id',$item->callback_id)->first();
            if($project) {
                $newdata = AsteriskCall::where('my_company_id', $project->my_company_id)->where('unique_id', $item->callback_id)->first();
                if (!$newdata) {
        ...
        foreach ($companys as $company) {
            ...
            foreach($phones_unic as $pc) {
                ...
                foreach ($inpcalls as $item) {
                    ...
                    $newdata->ogidanie = $item->duration - $item->billsec;
                    $newdata->timing = $item->billsec;
                    $newdata->unique_id = $item->uniqueid;
$pro=Project::where('uniqueid', $item->uniqueid)->first();
if($pro){
    $newdata->project_id=$pro->id;
}
    ...
    public function deletecanal(Request $request){
        ...
        $metrika->where('src_old','cost');
        $metrika->where('user_id',$request->id)->delete();
