...
class FieldController extends Controller
{
    ...
    protected function grid()
    {
        ...
        $datas['stages']=Field_tip::all()->where('tip',0)->where('my_company_id',$user->my_company_id);
;$datas['title']='Доп поля сделок';
    ...
    {$user=Auth::user();
        ...
        $data['tipfields']=0;
        if(is_null($id)){
            ;$data['title']='Доп поля сделок';
            return view('setting.field.add',$data);

        }else{

            $data=Field_tip::where('my_company_id',$user->my_company_id)->findOrFail($id);$data['tipfields']=0;
      ...
      if($request->stageId==0) {
       ...
       /*     stageId:stageId,
        ...
        vields_values:vields_values,*/
            ...
            'field_id'=>$request->tip_fields,'my_company_id',$user->my_company_id
                ]);
...
return '0';
            ...
            if($get_field->is_text==0){
                ...
                Field_tip_param::where('field_tip_id',$request->stageId)->delete();
                $array=explode(',',$request->vields_values);
                for($i=0;$i<count($array);$i++){
