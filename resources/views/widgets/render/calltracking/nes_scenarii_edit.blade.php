    ...
    <div class="modal-dialog" >
        <form id="create_routing_form"><div class="modal-content"  style="height: 100vh">
            <!-- Заголовок модального окна -->
                ...
                <input type="hidden" name="setings-add-nomer"  id="setings-add-nomer_val" value="{{$rout->{'setings-add-nomer'} }}">
            ...
            <div class="modal-body">
                <div class="name-block-fixed">
                    <div class="col-xs-12" ><div class="h1-modal pos-left" style="width: 90%"><input type="text" class="form-control new-scenariy" placeholder="Новый сценарий" name="name" value="{{$rout->name}}"> <span></span></div>
                            ...
                            <div id="collapse-1" class="panel-collapse collapse" role="tabpane1" aria-labelledby="heading-1" aria-expanded="false" style="height: 0px;">
                                ...
                                <div class="panel-body">
                                    <div id="setings-add-nomer" class="cont-btn col-xs-12">
                                        <button type="button" data-id="tab-add-nomers" class="btn btn-primary   @if($rout->{'setings-add-nomer'}=='tab-add-nomers') active @endif">Добавить номера</button>
                                        <button type="button" data-id="tab-add-nomer" class="btn btn-primary @if($rout->{'setings-add-nomer'}=='tab-add-nomer') active @endif">Добавление номера</button>
                                        <button type="button" data-id="tab-my-nomer" class="btn btn-primary @if($rout->{'setings-add-nomer'}=='tab-my-nome') active @endif">Свои номера</button>
                                    </div>
                                    <div id="setings-add-nomer-content"  class="cont-btn-content col-xs-12">
                                        <span class="switchery-xs"><input checked type="checkbox" class="js-switch" data-id="" name="is_default"></span>

                                        <div id="tab-add-nomers" class="tab-content-block @if($rout->{'setings-add-nomer'}=='tab-add-nomers') active @endif">
                                        ...
                                        <div id="tab-add-nomer" class="tab-content-block  @if($rout->{'setings-add-nomer'}=='tab-add-nomer') active @endif">
                                        ...
                                        <div id="tab-my-nomer" class="tab-content-block  @if($rout->{'setings-add-nomer'}=='tab-my-nomer') active @endif">
                                            <div class="form__block">
                                            ...
                                            <div class="on_of_nambers">
                                                <div class="form__block form__block_select_neiros">
                                                    ...
                                                    <select class="form-control select4-list" name="ar_reditrect">
                                                        <option></option>
                                                        <option value="0" @if($rout->tip_redirect==0) selected @endif>Телефон</option>
                                                        <option value="1"  @if($rout->tip_redirect==1) selected @endif>SIP</option>
                                                        <option value="2"  @if($rout->tip_redirect==2) selected @endif>SIP Neiros</option>
                                                        
                                                    </select>
                                                </div>
                                                <div class="form__block ">

                                                    <input type="text" class="form-control form-control-text <? if($rout->tip_redirect==1 || $rout->tip_redirect==2 || $rout->tip_redirect==3){ echo 'form-control-text-number';} else {echo 'form-control-text-number-mask';} ?>" placeholder="+7 (___) ___ __ __" name="ar_phone_redirect" value="{{$rout->number_to}}">
                                                </div>
                        ...
                        <div class="panel panel-default">
                            ...
                            <div id="collapse-3" class="panel-collapse collapse" role="tabpane1" aria-labelledby="heading-3" aria-expanded="false" style="height: 0px;">
                                ...
                                <div class="panel-body">
                                    ...
                                    <div class="cont-btn-content col-xs-12" style="padding-top: 0px;">
                                        <div class="label-form-name" style="padding-bottom:10px;">Выберите тип коллтрекинга</div><input type="hidden" name="ar_tip_calltrack" class="ar_tip_calltrack" value="{{$rout->tip_calltrack}}">
                                        <div  class="cont-btn col-xs-12" id="type-calltrecing-cont">
                                            <button type="button"  class="btn btn-primary @if($rout->tip_calltrack==1) active @endif" data-val="1">Динамический</button>
                                            <button type="button"  class="btn btn-primary @if($rout->tip_calltrack==2) active @endif" data-val="2">Статический</button>
                                        </div>
                                                    ...
                                                    <div class="form__block">
                                                       <select class="form-control select5-list" name="ar_canals_dinamic[]">
                                                            ...
                                                            @foreach($witget_canals as $item)
                                                                <option value="{{$item->id}}" @if( $item->id==$routecanal[$i]) selected @endif>{{$item->name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
...
@else
                                            ...
                                            <div class="col-xs-12 add_sourse_block">
                                                <div class="add_sourse"><div class="cont__left">+</div><div class="cont_right">Добавить источник</div></div>
                                            </div>
                                            ...
                                            <div class="on_of_nambers">
                                                <div class="form__block">
                                                    ...
                                                    <select class="form-control select6-list" name="ar_class_replace_type[]">
                                                        ...
                                                        <option value="." @if($all_class_replace[$k]->ar_class_replace_type=='.') selected @endif>Class</option>
                                                        <option value="#" @if($all_class_replace[$k]->ar_class_replace_type=='#') selected @endif>id</option>
                                                    </select>
                                                </div>
                                                <div class="form__block ">

                                                    <input type="text" class="form-control form-control-text" placeholder="" name="ar_class_replace[]" value="{{$all_class_replace[$k]->ar_class_replace}}">
                                                </div>
...
@endif
                ...
                <div class="form-group footer-button">
                    <div class="col-xs-12">
                        ...
                        <button type="button" class="btn btn-primary  add_scenarij "  ><? if($rout->name != ''){echo 'Сохранить сценарий';}else{echo 'Создать сценарий';}?></button>
